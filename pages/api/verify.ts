...
import { withSession, contractAddress, addressCheckMiddleware, 
  pinataApiKey, pinataSecretApiKey } from "./utils";
import { NftMeta } from "@_types/nft";
import axios from "axios";

export default withSession(async (req: NextApiRequest & {session: Session}, res: NextApiResponse) => {
  if(req.method === "POST"){
      // request post for body need {address, signature, nft}
    try {
      const {body} = req
      ...
      // request get for create message and set session on cookies
    try {
